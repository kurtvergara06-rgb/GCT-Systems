"""CLI: Train the inventory demand-forecasting Random Forest model.

Usage:
    python -m inventory.train_model

Reads the feature matrix produced by ``prepare_training_data``, trains a
RandomForestRegressor (project-wide convention), evaluates it on a CHRONOLOGICAL
held-out test window, and saves:

    inventory/models/inventory_demand_rf.pkl
    inventory/models/inventory_demand_features.json
    inventory/models/inventory_demand_report.txt
    inventory/models/inventory_demand_state.json

Development prototype: the model is trained on SAMPLE data only.
"""
...
def main() -> int:
    ...
    if not csv_path.exists():
        ...
        result = InventoryModelResult(
            n_samples=0, message="No training data found.", source="sample"
        )
    ...
    result = train_inventory_model(df, source="sample")

    if result.trained:
        encoders = build_encoders(df)
        part_metadata = build_part_metadata(df)
        bus_metadata = build_bus_metadata(df)
        save_inventory_model(result, paths, encoders, part_metadata, bus_metadata)
    else:
        save_inventory_model(result, paths)
    save_state(result, paths)

    print("\n=== Inventory Model #4 training results (SAMPLE / DEVELOPMENT) ===")
    print(f"Data source:      sample (generated, NOT GCT operational data)")
    ...
    for name, imp in sorted(result.feature_importances.items(), key=lambda kv: -kv[1])[:8]:
        print(f"  {name:<30} {imp:.4f}")
    print(f"\nArtifacts written to: {paths['dir']}")
    print(f"State file: {paths['state']}")
    return 0
