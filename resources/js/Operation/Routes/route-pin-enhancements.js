document.addEventListener('DOMContentLoaded', () => {
    ...
    const markActive = (input) => {
        ...
        const group = input.closest('.ui-form-group');
        const stopRow = input.closest('.route-stop-item');
        const target = group || stopRow;
    ...
    const updatePinTitles = () => {
        routeStopList
            ?.querySelectorAll('input[name="stops[]"]')
            .forEach((input) => {
                const button = input
                    .closest('.route-stop-location-field')
                    ?.querySelector(':scope > .route-direct-pin-btn');
    ...
    routeStopList?.addEventListener('input', updatePinTitles);
    document.getElementById('addRouteStop')?.addEventListener('click', () => {
        window.setTimeout(updatePinTitles, 0);
    });
