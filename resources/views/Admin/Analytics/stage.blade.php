...
$domains = ['all'=>['All','fa-layer-group'],'fleet-trip'=>['Fleet & Trip','fa-route'],'fuel'=>['Fuel','fa-gas-pump'],'bus-health'=>['Bus Health','fa-heart-pulse'],'inventory'=>['Inventory','fa-boxes-stacked']];
@endphp
<x-layout.app title="FROMS - {{ $stageLabel }} Analytics" :assets="['resources/css/Admin/Analytics/fleet-trip.css','resources/css/Admin/Analytics/fleet-trip-redesign.css','resources/css/Admin/Analytics/fleet-trip-rankings.css','resources/css/Admin/Analytics/analytics-stage-hub.css']">
<div class="app"><x-layout.sidebar department="Admin" /><main class="main analytics-stage-page fleet-trip-page">
...
<a href="{{ route('analytics.stage',['stage'=>$stage],false) }}?{{ http_build_query(['domain'=>$key,'period'=>$period,'bus'=>$selectedBus !== 'all' ? $selectedBus : null]) }}" class="{{ $domain === $key ? 'active' : '' }}"><i class="fa-solid {{ $meta[1] }}"></i><span>{{ $meta[0] }}</span></a>
@endforeach
</nav>
...
<label><span>Bus</span><select name="bus"><option value="all">All Buses</option>@foreach($busOptions as $busNo)<option value="{{ $busNo }}" @selected(strtoupper($selectedBus)===$busNo)>{{ $busNo }}</option>@endforeach</select></label>
<button type="submit"><i class="fa-solid fa-filter"></i> Apply</button>
</form></section>
@include('Admin.Analytics.sections.' . $stage)
</main></div></x-layout.app>