@php
    ...
    $poly = $points->map(fn ($point) => $point['x'] . ',' . $point['y'])->implode(' ');
    $area = $poly !== '' ? $poly . ' 678,194 42,194' : '';
    ...
    <div class="app">
        ...
        <main class="main analytics-stage-page fleet-trip-page descriptive-analytics-page">
            <x-layout.topbar
                ...
                subtitle="What happened based on recorded operational data."
            />
            ...
            <section class="analytics-domain-toolbar descriptive-toolbar">
                <nav class="analytics-domain-tabs" aria-label="Descriptive analytics domains">
                    @foreach($tabs as $key => $tab)
                        <a
                            href="{{ url('/analytics/descriptive') }}?{{ http_build_query(['domain' => $key, 'period' => $period, 'bus' => $selectedBus !== 'ALL' ? $selectedBus : null]) }}"
                            class="{{ $domain === $key ? 'active' : '' }}"
                        >
                            <i class="fa-solid {{ $tab[1] }}"></i>
                            {{ $tab[0] }}
                        </a>
                    @endforeach
                </nav>
                    ...
                    <label>
                        ...
                        <select name="bus">
                            ...
                            @foreach($buses as $bus)
                                <option value="{{ $bus->bus_no }}" @selected($selectedBus === strtoupper($bus->bus_no))>
                                    {{ $bus->bus_no }}
                                </option>
                            @endforeach
                        </select>
            ...
            @if($domain === 'fuel')
                <section class="analytics-kpi-strip analytics-kpi-strip-three">
                    ...
                    <x-analytics.kpi
                        ...
                        small="Distance divided by recorded fuel"
                        icon="fa-gauge-high"
                        tone="purple"
                    />
                </section>
            @elseif($domain === 'bus-health')
                <section class="analytics-kpi-strip analytics-kpi-strip-three">
                    ...
                    <x-analytics.kpi label="Under Maintenance" :value="$underMaintenance" small="Needs attention" icon="fa-screwdriver-wrench" tone="yellow" />
            ...
            @elseif($domain === 'inventory')
                <section class="analytics-kpi-strip analytics-kpi-strip-three">
                    <x-analytics.kpi label="Well Stocked" :value="$inventoryHealthy" small="Above reorder threshold" icon="fa-boxes-stacked" tone="green" />
            ...
            @else
                ...
                <section class="analytics-main-grid analytics-main-grid-balanced descriptive-main-grid">
                    <article class="analytics-card analytics-reference-chart-card descriptive-trip-card">
                        ...
                        <div class="reference-line-chart">
                            <svg viewBox="0 0 720 230" preserveAspectRatio="none" role="img" aria-label="Processed trip activity chart">
                                <defs>
                                    <linearGradient id="tripFill" x1="0" y1="0" x2="0" y2="1">
                                        <stop offset="0%" stop-color="#2f6ee5" stop-opacity=".22" />
                                        <stop offset="100%" stop-color="#2f6ee5" stop-opacity=".02" />
                                    </linearGradient>
                                </defs>
                                ...
                                @if($area !== '')
                                    <polygon points="{{ $area }}" fill="url(#tripFill)" />
                                @endif

                                @if($points->count() > 1)
                                    <polyline points="{{ $poly }}" class="reference-chart-line" />
                                @endif

                                @foreach($points as $point)
                                    <circle cx="{{ $point['x'] }}" cy="{{ $point['y'] }}" r="5" class="reference-chart-dot" />
                                    <text x="{{ $point['x'] }}" y="{{ max(18, $point['y'] - 12) }}" text-anchor="middle" class="reference-chart-value">
                                        {{ $point['count'] }}
                                    </text>
                                    <text x="{{ $point['x'] }}" y="218" text-anchor="middle" class="reference-chart-label">
                                        {{ $point['label'] }}
                                    </text>
                                @endforeach
                            </svg>
                        ...
                        <div class="reference-chart-legend">
                    ...
                    <article class="analytics-card descriptive-availability-card">
                        ...
                        <div class="analytics-availability-layout">
                            <div class="availability-score">
                                <div class="availability-ring" style="--availability-angle: {{ min(360, max(0, $fleetAvailability * 3.6)) }}deg">
                ...
                @else
                    <section class="analytics-list-grid descriptive-ranking-only-grid">
                                ...
                                @forelse($routes as $route)
                                    <div class="refined-ranking-row">
                                        ...
                                        <div class="refined-ranking-main">
                                            ...
                                            <div class="refined-ranking-meta"><span><i class="fa-regular fa-clock"></i>{{ number_format($route->average_duration, 1) }} min avg.</span><span><i class="fa-solid fa-chart-pie"></i>{{ number_format($route->share, 1) }}%</span></div>
                                            <div class="metric-bar refined-metric-bar"><span style="width: {{ $route->progress }}%"></span></div>
                                        </div>
                                    </div>
                                @empty
                                ...
                                @forelse($busActivity as $bus)
                                    <div class="refined-ranking-row">
                                        ...
                                        <div class="refined-ranking-main">
                                            ...
                                            <div class="refined-ranking-meta"><span><i class="fa-solid fa-road"></i>{{ number_format($bus->distance, 1) }} km</span><span><i class="fa-solid fa-chart-pie"></i>{{ number_format($bus->share, 1) }}%</span></div>
                                            <div class="metric-bar refined-metric-bar"><span style="width: {{ $bus->progress }}%"></span></div>
                                        </div>
                                    </div>
                                @empty
