...
<section class="analytics-kpi-strip analytics-domain-kpi-four">
    <x-analytics.kpi label="Distance Traveled" :value="number_format($totalDistance, 1) . ' km'" small="Total recorded trip distance" icon="fa-road" />
    <x-analytics.kpi label="Trips Processed" :value="number_format($tripCount)" small="Total trip records" icon="fa-route" tone="green" />
    <x-analytics.kpi label="Average Speed" :value="number_format($averageSpeed, 1) . ' km/h'" small="Average while in motion" icon="fa-gauge-high" tone="purple" />
...
<section class="fleet-trip-domain-grid">
    <article class="analytics-card analytics-reference-chart-card descriptive-trip-card">
        <div class="descriptive-overview-card-heading"><div><h3>Processed Trip Activity</h3><p>Trip-record volume across the selected period.</p></div><span>{{ $periodLabel }}</span></div>
=======
...
>>>>>>> 9b5a5e2066a219eb694087d2798fee1b3cc1f62d
...
<<<<<<< HEAD
    <article class="analytics-card descriptive-availability-card">
        ...
        <div class="analytics-availability-layout"><div class="availability-score"><div class="fleet-css-donut" data-default-value="{{ number_format($fleetAvailability, 1) }}%" data-default-label="Active" data-active="{{ number_format($activePct, 2, '.', '') }}" data-maintenance="{{ number_format($maintenancePct, 2, '.', '') }}" data-inactive="{{ number_format($inactivePct, 2, '.', '') }}" style="--fleet-active: {{ number_format($activePct, 2, '.', '') }}%; --fleet-maintenance-end: {{ number_format($maintenanceEndPct, 2, '.', '') }}%;"><div class="fleet-css-donut-center"><strong>{{ number_format($fleetAvailability, 1) }}%</strong><span>Active</span></div><div class="fleet-css-donut-tooltip" aria-hidden="true"><strong></strong><span></span></div></div></div><div class="availability-breakdown"><div class="availability-row" data-donut-index="0" data-label="Active" data-value="{{ $activeBuses }}" data-percentage="{{ number_format($activePct, 1, '.', '') }}"><div><span class="availability-dot operational"></span><span>Active</span></div><strong>{{ $activeBuses }}</strong></div><div class="availability-row" data-donut-index="1" data-label="Under Maintenance" data-value="{{ $underMaintenance }}" data-percentage="{{ number_format($maintenancePct, 1, '.', '') }}"><div><span class="availability-dot maintenance"></span><span>Under Maintenance</span></div><strong>{{ $underMaintenance }}</strong></div><div class="availability-row" data-donut-index="2" data-label="Inactive" data-value="{{ $inactiveBuses }}" data-percentage="{{ number_format($inactivePct, 1, '.', '') }}"><div><span class="availability-dot inactive"></span><span>Inactive</span></div><strong>{{ $inactiveBuses }}</strong></div><div class="availability-total"><span>Total Buses</span><strong>{{ $totalBuses }}</strong></div></div></div>
    </article>
</section>

<section class="fleet-trip-ranking-grid">
