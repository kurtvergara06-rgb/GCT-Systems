...
<section class="diag-stack">
    ...
    <div class="fleet-main-grid">
        <article class="diag-card">
            <x-analytics.card-header class="diag-card-head" title="Why Fleet &amp; Trip Performance Shifted" description="Diagnostic evidence derived from route medians, speed, duration, and idling." />
        ...
        <article class="diag-card">
            <x-analytics.card-header class="diag-card-head" title="Top Diagnostic Drivers" description="Supported factors ranked by observed record count." />
            <div class="diag-list">
    ...
    <div class="fleet-lower-grid">
        <article class="diag-card"><x-analytics.card-header class="diag-card-head" title="Route Bottleneck Analysis" description="Routes represented most often in review records." />@if($routeReview->isEmpty())<div class="diag-empty">No route-level review evidence for the selected period.</div>@else<div class="diag-bars">@php $m=max(1,(int)$routeReview->max('score')); @endphp @foreach($routeReview as $row)<div class="diag-bar-row"><span>{{ $row->route }}</span><div class="diag-bar-track"><i class="diag-bar-fill" style="width:{{ ($row->score/$m)*100 }}%"></i></div><b>{{ $row->count }}</b></div>@endforeach</div>@endif</article>
        <article class="diag-card"><x-analytics.card-header class="diag-card-head" title="Bus Utilization Distribution" description="Trip contribution by the most active buses in the selected data." />@if($busActivity->isEmpty())<div class="diag-empty">No bus activity records available.</div>@else<div class="diag-bars">@foreach($busActivity as $bus)<div class="diag-bar-row"><span>{{ $bus->bus }}</span><div class="diag-bar-track"><i class="diag-bar-fill green" style="width:{{ min(100,$bus->share) }}%"></i></div><b>{{ number_format($bus->trips) }}</b></div>@endforeach</div>@endif</article>
        <article class="diag-card"><x-analytics.card-header class="diag-card-head" title="Route Performance Context" description="Trip count and average duration for leading routes." />@if($routes->isEmpty())<div class="diag-empty">No route records available.</div>@else<div class="diag-list">@foreach($routes as $route)<div class="diag-list-row"><span class="diag-list-rank">{{ $loop->iteration }}</span><div><strong>{{ $route->label }}</strong><small>{{ number_format($route->average_duration,1) }} min avg duration</small></div><span class="diag-list-value">{{ number_format($route->trips) }} trips</span></div>@endforeach</div>@endif</article>
    </div>

    <article class="diag-card diag-evidence-card">
        <x-analytics.card-header class="diag-card-head" title="Diagnostic Evidence Table" description="Highest-scoring records with supported delay, movement, or idling factors." />
    ...
    <div class="diag-grid-3-equal">
        ...
        <article class="diag-card"><x-analytics.card-header class="diag-card-head" title="Priority Investigations" /><ol class="diag-priority-list"><li>Review delayed records on routes with sufficient baseline coverage.</li><li>Inspect buses with repeated high-idle records.</li><li>Compare slow-movement records against route and time context.</li></ol></article>
        <article class="diag-card"><x-analytics.card-header class="diag-card-head" title="Data Support" description="Evidence used by this diagnostic page." /><div class="diag-list"><div class="diag-list-row"><span class="diag-list-rank"><i class="fa-solid fa-route"></i></span><div><strong>Processed GPS Trips</strong><small>Trips analyzed</small></div><span class="diag-list-value">{{ number_format($diagnostic->fleet->trip_count) }}</span></div><div class="diag-list-row"><span class="diag-list-rank"><i class="fa-solid fa-chart-line"></i></span><div><strong>Route Baseline Coverage</strong><small>Supported comparison records</small></div><span class="diag-list-value">{{ number_format((float)($d->baseline_coverage_percent ?? 0),1) }}%</span></div></div></article>
    </div>
</section>
