...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (Schema::hasColumn('inventory_items', 'item_name')) {
            DB::statement("UPDATE inventory_items SET parts_name = item_name WHERE parts_name IS NULL OR parts_name = ''");
        }

        if (Schema::hasColumn('inventory_items', 'quantity')) {
            DB::statement("UPDATE inventory_items SET on_hand = quantity WHERE on_hand = 0");
        }

        if (Schema::hasColumn('inventory_items', 'stock')) {
            DB::statement("UPDATE inventory_items SET on_hand = stock WHERE on_hand = 0");
        }

        if (Schema::hasColumn('inventory_items', 'stock_quantity')) {
            DB::statement("UPDATE inventory_items SET on_hand = stock_quantity WHERE on_hand = 0");
        }
